//! Cross-validation of the formula engine against patsy design matrices frozen
//! in `tests/fixtures/formula.json` (regenerate with
//! `python3 tools/reference/gen_formula.py`).
...
fn load() -> Value {
    ...
    let s = fs::read_to_string(p).expect("fixture present (run tools/reference/gen_formula.py)");
...
fn formula_matches_patsy() {
    ...
    for c in fx["cases"].as_array().unwrap() {
        ...
        for ((i, j), &g) in out.design.indexed_iter() {
            ...
            assert!(
                (g - w).abs() <= TOL,
                "{name}: design[{i}][{j}] = {g}, want {w} (|diff| {:.3e})",
                (g - w).abs()
            );
        }
