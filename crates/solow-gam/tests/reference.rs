//! Cross-validation of the penalized B-spline GAM against golden reference
//! values frozen in `tests/fixtures/gam.json`.

use ndarray::{Array1, Array2};
use serde_json::Value;
use solow_gam::{BSplines, GamResults, GlmGam};
use solow_glm::Family;
use std::fs;

fn load() -> Value {
    let p = concat!(env!("CARGO_MANIFEST_DIR"), "/../../tests/fixtures/gam.json");
    let s = fs::read_to_string(p).expect("fixture present (run tools/reference/gen_gam.py)");
...
fn gam_matches_reference() {
    ...
    for c in fx["cases"].as_array().unwrap() {
        ...
        // --- Basis construction (closed-form): tight tolerance. ------------
        ...
        // --- Fit and verify estimated quantities. --------------------------
        let res: GamResults = GlmGam::new(y, &x, df, degree, alpha, family)
            .unwrap()
            .fit()
            .unwrap();
        assert!(res.converged, "{name}: P-IRLS did not converge");

        check_vec(name, &res.params, exp, "params", 1e-9);
        check_vec(name, &res.fittedvalues, exp, "fittedvalues", 1e-9);
        check_vec(name, &res.edf, exp, "edf", 1e-9);
        check_scalar(name, res.edf_total, exp, "edf_total", 1e-9);
        check_scalar(name, res.edf_total, exp, "hat_matrix_trace", 1e-9);
        check_scalar(name, res.scale, exp, "scale", 1e-9);
        check_scalar(name, res.deviance, exp, "deviance", 1e-9);
        check_scalar(
            name,
