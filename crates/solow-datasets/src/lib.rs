...
pub fn make_classification(
    ...
    if n_samples == 0 || n_features == 0 {
        return Err(Error::Value(
            "make_classification: n_samples/n_features must be ≥ 1".into(),
        ));
    }
    if n_classes < 2 {
        return Err(Error::Value(
            "make_classification: n_classes must be ≥ 2".into(),
        ));
    }
...
pub fn make_regression(
    ...
    if n_samples == 0 || n_features == 0 {
        return Err(Error::Value(
            "make_regression: n_samples/n_features must be ≥ 1".into(),
        ));
    }
    let mut rng = Lcg::new(seed);
    let mut x = Array2::<f64>::zeros((n_samples, n_features));
...
pub fn make_circles(
    ...
    if !(0.0..1.0).contains(&factor) {
        return Err(Error::Value(
            "make_circles: factor must be in (0, 1)".into(),
        ));
    }
    let mut rng = Lcg::new(seed);
    let mut x = Array2::<f64>::zeros((n_samples, 2));
...
pub fn make_swiss_roll(
    ...
    if n_samples == 0 {
        return Err(Error::Value(
            "make_swiss_roll: n_samples must be ≥ 1".into(),
        ));
    }
    let mut rng = Lcg::new(seed);
...
pub fn make_low_rank_matrix(
    ...
    if n_samples == 0 || n_features == 0 || effective_rank == 0 {
        return Err(Error::Value(
            "make_low_rank_matrix: sizes must be ≥ 1".into(),
        ));
    }
    let mut rng = Lcg::new(seed);
...
pub fn load_iris() -> (Array2<f64>, Array1<usize>, Vec<&'static str>) {
    ...
    (
        x,
        y,
        vec!["sepal length", "sepal width", "petal length", "petal width"],
    )
}
